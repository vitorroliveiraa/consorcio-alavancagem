      ...
      <CardHeader className="pb-4 px-3 sm:px-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-[10px] sm:text-sm">Evolucao por mes com reajuste INCC anual</CardDescription>
          </div>
        </div>
          ...
          <TabsContent value="sorteio" className="mt-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 p-3 rounded-lg bg-secondary/30 border border-border">
              <div className="flex items-center gap-2">
                <Label className="text-xs sm:text-sm text-muted-foreground whitespace-nowrap">Parcela:</Label>
                <Select value={tipoParcela} onValueChange={(v) => onTipoParcelaChange(v as TipoParcela)}>
                ...
                <Table>
                  <TableHeader className="sticky top-0 bg-card z-10">
                    <TableRow className="border-b border-border">
                      <TableHead className="w-12 sm:w-16 text-center bg-card text-xs sm:text-sm">Mes</TableHead>
                      <TableHead className="text-right bg-card text-xs sm:text-sm">Parcela</TableHead>
                      <TableHead className="text-right bg-card text-xs sm:text-sm whitespace-nowrap">Tot. Investido</TableHead>
                      <TableHead className="text-right bg-card text-xs sm:text-sm whitespace-nowrap">Cred. Liquido</TableHead>
                      <TableHead className="text-right bg-card text-xs sm:text-sm whitespace-nowrap">Recompra</TableHead>
                      <TableHead className="text-right bg-card text-xs sm:text-sm whitespace-nowrap">Lucro</TableHead>
                      <TableHead className="text-right w-16 sm:w-24 bg-card text-xs sm:text-sm">ROI</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {sorteio.map((r, index) => {
                      const isDestaque = r.mes === melhorRoi.mes && melhorRoi.modalidade === 'sorteio'
                      ...
                      return (
                        <React.Fragment key={`sorteio-${r.mes}`}>
                          {mudouAno && (
                            <TableRow key={`year-${anoAtual}`} className="bg-accent/10 border-y-2 border-accent/30">
                          ...
                          >
                            ...
                            <TableCell className="text-right font-mono text-[11px] sm:text-sm">
                              <span className={r.anoReajuste ? 'text-primary font-medium' : ''}>
                            ...
                            <TableCell className="text-right font-mono text-[11px] sm:text-sm">
                              <span className={r.lucroLiquido >= 0 ? 'text-success' : 'text-destructive'}>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Label className="text-xs sm:text-sm text-muted-foreground whitespace-nowrap">Lance:</Label>
                    <span className="font-semibold text-xs sm:text-base text-foreground">{formatarMoeda(lanceConfig.valorLance)}</span>
                    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                      <DialogTrigger asChild>
                        <Button variant="outline" size="sm" className="h-6 sm:h-7 gap-1 sm:gap-1.5 text-[10px] sm:text-xs bg-transparent" onClick={openDialog}>
                    ...
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Defina o valor do lance e como ele será utilizado na contemplação.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => handleLanceValueChange(Number(e.target.value))}
                            placeholder="0,00"
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="embutido">Embutido (parte do crédito)</SelectItem>
                              <SelectItem value="pago">Pago (dinheiro próprio)</SelectItem>
                              <SelectItem value="dividido">Dividido (ambos)</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setTempLanceConfig({
                                    ...
                                    valorEmbutido: tempLanceConfig.valorLance - Number(e.target.value),
                                  })
                                }
                              />
                            </div>
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setTempLanceConfig({
                                    ...
                                    valorPago: tempLanceConfig.valorLance - Number(e.target.value),
                                  })
                                }
                              />
                            </div>
                        ...
                        <div className="p-3 rounded-lg bg-muted/50 border">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {(tempLanceConfig.tipoLance === 'pago' || tempLanceConfig.tipoLance === 'dividido') &&
                            ...
                            {(tempLanceConfig.tipoLance === 'embutido' || tempLanceConfig.tipoLance === 'dividido') &&
                              tempLanceConfig.valorEmbutido > 0 && (
                                <Badge className="bg-destructive/20 text-destructive border-destructive/30 hover:bg-destructive/20">
                                  Emb: {formatarMoeda(tempLanceConfig.valorEmbutido)}
                                </Badge>
                              )}
                          </div>
                        </div>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  ...
                  {(lanceConfig.tipoLance === 'embutido' || lanceConfig.tipoLance === 'dividido') &&
                ...
                <Table>
                  ...
                  <TableBody>
                    {lancefixo.map((r, index) => {
                      const isDestaque = r.mes === melhorRoi.mes && melhorRoi.modalidade === 'lance-fixo'
                      const isPontoVirada = r.mes === pontoViradaLanceFixo
                      const anoAtual = Math.ceil(r.mes / 12)
                      const anoAnterior = index > 0 ? Math.ceil(lancefixo[index - 1].mes / 12) : 0
                      const mudouAno = index > 0 && anoAtual > anoAnterior

                      return (
                        <React.Fragment key={`lancefixo-${r.mes}`}>
                          {mudouAno && (
                            <TableRow key={`year-lf-${anoAtual}`} className="bg-accent/10 border-y-2 border-accent/30">
                              <TableCell colSpan={7} className="py-1.5 text-center">
                                <span className="text-xs font-semibold text-accent uppercase tracking-wider">
                                  Ano {anoAtual}
                                </span>
